...
## 에이전트 가이드 원칙 (사용자 직접 구현 지원)
- 에이전트는 프로젝트 소스 코드(.cs 등)를 직접 생성하거나 수정하지 않는다.
- 에이전트는 사용자가 직접 코드를 작성 및 완성할 수 있도록 단계별 계획(Implementation Plan)을 설계하고, 코드 힌트(클래스 구조, 사용해야 할 Unity API, 논리적 흐름 등)를 제공한다.
- 모든 계획 및 가이드는 유니티 2D 물리 규칙과 프로젝트 Specs를 준수해야 한다.
